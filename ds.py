...
if os.name=="nt":
	DATABASE="C:\\Users\\iezyq\\Desktop\\repos\\werrefy\\database.db"
else:
...
@bot.command(name="установить-зп" , help="Назначает определенный доход к роли. назн-доход <пинг-роли> <доход>")
@whos()
...
@scomes.error
async def on_scomes_error(ctx, error):
	if isinstance(error, discord.ext.commands.errors.MissingRequiredArgument):
		...
		embed.set_footer(text=f"{ctx.author.name} ◊ Rawec", icon_url=str(ctx.author.display_avatar))
		await ctx.send(embed=embed)

@bot.command(name="удалить-зп", help="₽убрать-доход <номер_строчки_в_лзп")
@whos()
...
@bot.command(name="придумать-вещь", help="Создаёт ещё одну вещь.")
@whos()
...
async def zp(ctx, member: discord.Member=None):
	q=0
			...
			for row in c.fetchall():
				if row is not None:
					...
					for a in  c.execute("SELECT roleId FROM {}_incomes WHERE roleId={}".format(uss, role.id, )).fetchone():
						...
						conn.commit()
		e=discord.Embed(title="**:incoming_envelope:¿~ Доход получен! ~¿:incoming_envelope:**", description=f"**{ctx.author.mention}, вы получили  доход ПО ВСЕМ должностям, которые вы занимаете в сумме\n {q}\n В случае ошибок обращаться к администрации сервера.**", color=discord.Color.from_rgb(250,161,204))
